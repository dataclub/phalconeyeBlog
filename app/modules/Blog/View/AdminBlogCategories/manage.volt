...
{% block title %}{{ "Manage categories"|i18n }}{% endblock %}
...
{% block head %}
    ...
    <script type="text/javascript">
        var menuItemsData = {
            ...
            'link_create': '{{ url(['for':'admin-module-blog-categories-create-item'])}}',
            'link_edit': '{{ url(['for':'admin-module-blog-categories-edit-item'])}}',
            'link_delete': '{{ url(['for':'admin-module-blog-categories-delete-item'])}}',
            'link_order': '{{ url(['for':'admin-module-blog-categories-order'])}}'
...
{% block content %}
    ...
    <div class="span12">
        <div class="menu_manage_header">
            <h3>
                <a href="{{ url("admin/module/blog/categories") }}">{{ "Categories" |i18n }}</a> >
                {% if parent is defined %}
                    {% for p in parents %}
                        <a href="{{ url(['for':'admin-module-blog-categories-manage']) }}{{ categories.id }}{% if p.parent_id is not null %}?parent_id={{ p.parent_id }}{% endif %}"
                           class='btn'>{{ p.title }}</a>
                        >
                    {% endfor %}
                {% endif %}
                {{ "Manage categories" |i18n }}
                "{{ categories.name }}"
            </h3>
                ...
                {% for item in items %}
                    <li data-item-id="{{ item.id }}">
                        <div class="item_title">
                            ...
                            {{ item.title }}
                            | {{ 'Items: '|i18n }}{{ item.getCategorieItems() ? item.getCategorieItems().count() : 0 }}
                        </div>
